...
[McpServerToolType]
public static class SupplierTool
{
    private const string entity = "supplier";
    private const string entityPlural = "suppliers";
    ...
    public static ValueTask<List<Supplier>> GetAll_Suppliers(
        HttpClient httpClient,
        CancellationToken cancellationToken)
    => ToolHelpers.GetAll<Supplier>(httpClient, entity, JsonSerializerOptions, cancellationToken);
    ...
    public static ValueTask<PagedList<Supplier>?> GetPaged_Suppliers(
        HttpClient httpClient,
        ...
        [Description($"The field to order the results by. It can only be null or empty for SupplierId, can only be companyName, contactName, contactTitle, address, city, region, postalCode, country")] string orderBy,
        ...
        CancellationToken cancellationToken)
    => ToolHelpers.GetPaged<Supplier>(httpClient, entity, JsonSerializerOptions, skip, take, orderBy, isAscending, filter, cancellationToken);
    ...
    public static ValueTask<Supplier?> GetById_Supplier(
            HttpClient httpClient,
            [Description($"id of the {entity}")] string id,
            CancellationToken cancellationToken)
    => ToolHelpers.GetById<Supplier, string>(httpClient, entity, JsonSerializerOptions, id, cancellationToken);

    [McpServerTool, Description($"Saves the {entity} to the API and return back to the client as {entity} details. Only the properties that are not null will be saved. If the {entity} does not exist, it will be created. SupplierId is optional and can be null or 0. If SupplierId is not provided, a new {entity} will be created with a new id. CompanyName is required and cannot be null or empty. ContactName, ContactTitle, Address, City, Region, PostalCode, Country are optional and can be null.")]
    public static ValueTask<Supplier> Save_Supplier(
        HttpClient httpClient,
        [Description($"The {entity} to save.")] Supplier e,
        CancellationToken cancellationToken)
    ...
    public static ValueTask Delete_Supplier(
        HttpClient httpClient,
